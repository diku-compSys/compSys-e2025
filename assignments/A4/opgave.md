# Simulatoropgaven (A4, CompSys 2025)

Finn Schiermer Andersen, november-december 2025

version 0.1

Denne opgave stilles mandag den 24. november og skal afleveres kl 16 søndag den 14. december.

Opgaven består af to delopgaver, 

1: implementation af en RISC-V simulator og 
2: undersøgelse af hopforudsigere.
...
skrevet i C og du kan bruge krydscompileren introduceret i forelæsningen 19/11 (se slides
i "materials/25_11_19_tiny_riscv_setup/slides.pdf"), til at oversætte dem til et ELF-formatet
...
read_elf.c, read_elf.h: - indlæsning af riscv .elf fil
...
Du finder filerne i kataloget "assignments/A4/src" i kursets offentlige repo.

Til vurdering af forudsigere udleveres nogle på forhånd oversatte programmer.
(Vi udleverer oversatte programmer så vi er sikre på at alle simulerer det samme)
Disse programmer skal simuleres med tilhørende inddatasæt. Du finder kildetekster,
elf-filer (oversatte programmer) og inddatasæt i kataloget "assignments/A4/predictor-benchmarks"

~~~
erat.elf: - Ingen inddata.
fib.elf: - Angiv "32" som argument på kommandolinien
radix.elf: - Udpak filen a.in.gz - angiv "a.in a.out" som argument på kommandolinien
~~~

Bemærk at simulationerne indeholder mange millioner instruktioner.
...
Den udleverede (ufærdige) disassembler/simulator tager en ELF (extension: ".elf") fil som første argument.
...
~~~
./sim myprog.elf -d                # disassembler ELF fil til standard output
./sim myprog.elf                   # simulering og ingen logning
./sim myprog.elf -l log            # simulering for hver eneste instruktion
./sim myprog.elf -s log            # simulering og derpå logning af afsluttende opsamling (antal instruktioner, simulationstid)
./sim fib.elf -l log -- 5          # simuler fib.elf med input 5, log udførelsen til "log"
~~~
...
~~~
./sim fib.elf -- 7                  # fib(7), ingen logning
./sim fib.elf -l log -- 7           # fib(7), logning for indlæsning af dis-fil og for hver eneste instruktion
./sim fib.elf -s log -- 7           # fib(7), logning af afsluttende opsamling (antal instruktioner, simulationstid)
~~~
...
Vi bruger en krydsoversætter til at frembringe ".elf" filer fra 'C' eller fra
...
### Fra riscv assembler til .elf fil
...
Husk at inkludere din AI-deklaration i din indsendelse.
